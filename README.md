...
## 2. Methodology: Feature Engineering and Stochastic Modeling
...
* **Peakiness (PP)**: Defined as the ratio of the maximum power to the mean power of the waveform. Specular reflections from leads yield significantly higher PP values compared to the diffuse scattering from sea ice.
* **Stack Standard Deviation (SSD)**: Derived from the Doppler stack, this parameter measures the variation in backscatter as a function of the look angle, providing further distinction in scattering mechanisms.

### 2.2 Unsupervised Learning via Gaussian Mixture Models (GMM)
...
To ensure the model's robustness, we further validated the stability across sub-sampled individual waveforms from diverse geographical locations. The consistent shape of the returns within each cluster confirms that the selected features are stable indicators of surface type regardless of localized signal noise.
